...
export default function StaffNavbar() {
  ...
  const handleUpload = async () => {
    ...
    try {
      setUploading(true);
      const res = await axiosinstance.post('/staff/upload-profile-pic', formData, {
        headers: {
          ...
          >
            <img
              src={
                preview ||
                (profilePic
                  ? `/staff/uploads/${profilePic}`
                  : `https://ui-avatars.com/api/?name=${encodeURIComponent(staff.name || 'Staff')}&background=random&size=128`)
              }
              alt="Profile"
              className="h-8 w-8 rounded-full object-cover border"
            />
            <span className="text-gray-700 dark:text-white">{staff.name || 'Staff'}</span>
            ...
            <div className="absolute top-12 right-0 w-64 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-md shadow-lg z-50 p-4 space-y-2">
              ...
              <div className="flex items-center space-x-2">
                <div className="w-10 h-10 rounded-full overflow-hidden border">
                  <img
                    src={preview || (profilePic ? `/staff/uploads/${profilePic}` : `https://ui-avatars.com/api/?name=${staff.name}&background=random&size=128`)}
                    alt="Preview"
